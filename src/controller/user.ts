...
import { pgsqlDataSource } from '@src/typeorm'
...
export default class UserController {
  ...
  public static async getUsers(ctx: Context): Promise<void> {
    ...
    const userRepository: Repository<User> = pgsqlDataSource.getRepository(User)
  ...
  public static async getUser(ctx: Context): Promise<void> {
    ...
    // load user by id
    const user: User | undefined = await userRepository.findOneBy({
      id: +ctx.params.id || 0
    })
  ...
  public static async createUser(ctx: Context): Promise<void> {
    ...
    userToBeSaved.name = requestBody.name
    userToBeSaved.email = requestBody.email
  ...
  public static async updateUser(ctx: Context): Promise<void> {
    ...
    userToBeUpdated.name = requestBody.name
    userToBeUpdated.email = requestBody.email
  ...
  public static async deleteUser(ctx: Context): Promise<void> {
    // get a user repository to perform operations with user
    const userRepository = pgsqlDataSource.getRepository(User)

    // find the user by specified id
    const userToRemove: User | undefined = await userRepository.findOneBy({
      id: +ctx.params.id || 0
    })
    if (!userToRemove) {
      // return a BAD REQUEST status code and error message
